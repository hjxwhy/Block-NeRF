...
def get_camera_frustum(img_size, K, W2C, frustum_length=0.5, color=[0., 1., 0.]):
    ...
    frustum_points = np.array([[0., 0., 0.],  # frustum origin
                               ...
                               [half_w, 0, 0],
                               [0, half_h, 0], 
                               [0, 0, frustum_length]])  # bottom-left image corner
    frustum_lines = np.array([[0, i] for i in range(
        1, 5)] + [[i, (i + 1)] for i in range(1, 4)] + [[4, 1]] + [[0, i] for i in range(5, 8)])
    frustum_colors = np.tile(np.array(color).reshape(
        (1, 3)), (frustum_lines.shape[0]-3, 1))
    coords_colors = np.array([[1., 0, 0], [0, 1., 0], [0, 0, 1.]])
    frustum_colors = np.concatenate([frustum_colors, coords_colors])
...
def frustums2lineset(frustums):
    N = len(frustums)
    merged_points = np.zeros((N * 8, 3))  # 5 vertices per frustum
    merged_lines = np.zeros((N * 11, 2))  # 8 lines per frustum
    merged_colors = np.zeros((N * 11, 3))  # each line gets a color
...
def visualize_cameras(colored_camera_dicts, sphere_radius, geometry_file=None, geometry_type='mesh'):
    ...
    for color, camera_dict in colored_camera_dicts:
        ...
        for img_name in sorted(camera_dict.keys()):
            K = np.array(camera_dict[img_name]['K'])
